<script setup>
  ...
  const tabs = [
    ...
    { id: "android", label: "手机版" },
    { id: "fangames", label: "优秀原创作品" },
    { id: "assets", label: "关卡创作资源" },
    { id: "softendo", label: "周边游戏作品" }
  ]
...
<MfGamesEntry name="Mario Forever Remake" /><MfGamesEntry name="Mario Forever Advance Remake" />
</div>

<div v-if="currentTab === 'ce'">
...
目前，在安卓设备上游玩 Mario Forever，主要有两种方式：使用专门适配的 Android 版本，或者通过模拟器运行电脑版。

其中，专门适配手机的 Mario Forever 游戏数量仍然较少。已知存在两个不同的原版 Mario Forever 的 Android 移植版：
...
由于许多 Mario Forever 作品基于 Clickteam Fusion 开发，部分作品在模拟器（如 Winlator）中可能出现画面显示故障，甚至无法运行。即使游戏能够顺利启动，其中的绝大多数也并未原生支持游戏手柄，玩家往往需要配置屏幕虚拟按键，手动配置过程较为繁琐。相对而言，Mario Forever: Community Edition 这类 Godot 引擎的游戏在模拟器中的运行效果良好，且支持手柄。

推荐使用“[盖世游戏](https://gamehub.xiaoji.com/)”模拟器游玩 Mario Forever 作品。要想正常运行 Clickteam Fusion 2.5 制作的绝大部分作品，需使用盖世游戏 **6.0.2 及更早版本（推荐 5.3.5）。不要使用最新版**，最新版盖世游戏运行这部分作品时会出现黑屏 Bug。

**参考教程：https://www.marioforever.net/thread-3885-1-1.html**
</div>

<div v-if="currentTab === 'fangames'">

## Mario Forever 优秀原创作品
...
<MfGamesEntry name="Mario Forever: Minus Worlds" defaultVer="v1.8 bugfix" />
...
游戏融入了许多新颖元素，如：选关式大地图，取自《Super Mario Bros. 3》中的浣熊装、幽灵装，自创横向弹簧、炮台黑刺猬等。

推荐 **2012-12-29 版本**；另有国外网友制作的 Slightly-Fixed 版本，该版本修复了部分 bug。
...
本作是基于 Buziol Games 于 2005 年制作的射击游戏 Obliterator 的换皮扩充版，另有两个不同素材的换皮扩充版 Arcade Sniper 和 Dragon Ball Arcade。
...
游玩本作需使用 [Adobe Flash Player](https://file.marioforever.net/Mario%20Forever/Softendo%20%E5%85%B6%E4%BB%96%E6%B8%B8%E6%88%8F%E4%B8%8B%E8%BD%BD/flash/swf/adobe-flash-player/) 或 [Ruffle](https://ruffle.rs/)。
...
**A:** 制作 Mario Forever 作品需要使用游戏开发软件（目前使用最为广泛的是 Clickteam Fusion 2.5，简称 CTF 或 MMF，也包括但不限于其他游戏开发软件，如 GameMaker、Godot 等）和游戏制作模板（Rainbow Engine，Ultra Edition 等）。
